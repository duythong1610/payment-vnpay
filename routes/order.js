...
router.post("/create_payment_url", function (req, res, next) {
  ...
  res.json({ vnp: vnpUrl, return: returnUrl });
  res.redirect(vnpUrl);
});
...
router.get("/vnpay_return", function (req, res, next) {
  ...
  //   res.cookie("code", vnp_Params["vnp_ResponseCode"]);
  // }
  ...
  if (secureHash === signed) {
    ...
    // // res.redirect(`http://localhost:3000/${vnp_Params["vnp_ResponseCode"]}`);
    // res.redirect(`http://localhost:3002`);
    // res.cookie("codee", "alo");
  } else {
    res.status(400);
  }
